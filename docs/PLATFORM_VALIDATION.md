...
macOS 的 DebugProfile 与 Release entitlements 含安全存储插件要求的 `keychain-access-groups` 空数组；Team/App Group 未被硬编码。插件具体要求以锁定版本 flutter_secure_storage 10.3.1 的文档为依据，实际持久化仍须同签名标识的原生冷启动验证。
...
开发执行位置、工具版本、触发和产物见 `.pdec/README.md`。`scripts/validate_desktop.py` 默认只做前置检查，`--execute` 才编译。Windows 构建产物只上传为保留 7 天的开发 artifact，不发布正式版本。
