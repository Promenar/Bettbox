...
| 版本 | v0.7（共享客户端与平台验证） |
| 日期 | 2026-09-22 |
| 变更 | 邀请服务隔离验证、桌面收银适配、macOS 12 工程基线和原生构建验收。 |
...
- 跳转型（type=1，收银台 URL）：Android 内嵌 WebView 打开并提供浏览器入口；桌面端调系统浏览器打开。初始地址仅接受无 userinfo 的 HTTPS URL，拉起失败显示本地化提示。平台分支已有 Widget 测试，真实支付仍需商户环境验收。
- 免费订单（type=-1）直接按成功处理。
...
**F-USER-5（P0）邀请返利**：沿用 Xboard 后台奖励与结算规则，客户端在已登录账户页提供邀请入口。
- 读取可用邀请码与已邀请注册人数、累计佣金、确认中佣金、可用佣金。金额单位为分，展示币种来自后台，不在客户端计算奖励。
- 复用已有邀请码；没有可用码时由用户主动生成。创建请求有副作用，禁止页面加载自动创建或失败后自动重放。
- 链接和二维码使用同一网页注册地址：后台 `guest/comm/config.app_url` 对应的 HTTPS 站点加内置主题 `/#/register?code=` 路由；允许部署在网站子路径。分享内容不含认证凭据或订阅地址，不使用 API 故障转移域名替代公开网站地址。
- 新用户在网页完成注册并绑定邀请关系，再由网站承接客户端下载；网站主题、归属结果与下载引导需部署联调验收。App 注册页保留手工输入邀请码能力。
- 网站配置缺失、带认证信息、非 HTTPS 或含查询/片段时，提示网页暂不可用，保留复制邀请码与收益展示；异常响应不得显示为零收益。网络失败可手工刷新，登出/换账号清除旧账户展示。
- 客户端提供复制邀请链接与邀请码，并渲染二维码；首期不含系统分享面板、二维码图片导出、提现或佣金转余额。提现申请（`ticket/withdraw`）保留为后续独立需求。
- 接口按 `cedar2025/Xboard@4f48e61a2cbc6db5338872b6bdb45ef954ec1256` 核对；部署实例尚待验证，不能将上游源码与模拟测试视为真实返佣到账证据。
...
| 邀请 | `/user/invite/fetch`、`/user/invite/save`（创建有副作用） | GET | P0 |
...
**域名迁移到包商 Vercel China 反代（2026-09-04 完成）**：新域名 `cloud.bingcn.site`（面板）/ `api.bingcn.site`（订阅）/ `origin.bingcn.site`（Vercel 回源，DNS-01）；三域名均托管于 Cloudflare，ding 证书链：origin 由服务器 Caddy 用 **Cloudflare DNS-01 + Let's Encrypt** 自动签续（自定义镜像 `caddy-cloudflare`，xcaddy 集成 `caddy-dns/cloudflare`；`CF_DNS_API_TOKEN` 存 `/opt/xboard-test/.env`）；cloud/api 由 **Vercel China 边缘自动签发**。**运维要点**：① Caddy 移除 `auto_https off` 后自动开启 80 跳转撞 indenp nginx → 改用 `auto_https disable_redirects`（保留证书自动签发、不监听 80）；② 同端口多站点必须**显式写主机名**（`https://:8443` catch-all 会以默认策略吞掉所有 SNI 连接，导致 origin 请求拿旧自签证书）；③ 重写 Caddyfile 时勿丢 `/bootstrap.json` 静态路由；④ Vercel rewrites 回源要带端口 `https://origin.bingcn.site:8443/:path*`；⑤ 客户端引导源域名池已并入 `https://api.bingcn.site`（旧域保留冗余），`app_url=https://cloud.bingcn.site`（订阅 URL 随访问域名生成）。**后台 secure_path 生效需重启容器**（2026-09-04 实测）：Xboard `Setting` 类 `loadedSettings` 为实例级缓存，Octane worker 常驻进程内只加载一次——后台修改「安全路径」等设置后路由仍按旧值匹配（`route:list` 显示新值而 HTTP 404），重启 xboard 容器即生效；入口 `https://cloud.bingcn.site/codecenter`（旧域同路径）。

**SSR 上游不兼容结论（2026-09-04）**：Hitun 机场（rss-node）clash 订阅 63 节点全为 SSR（ShadowsocksR）；实测 `target=ss/trojan` 空响应、`target=vmess/vless/hysteria2` 返回 400——该机场无 mihomo 可用格式，与 Bettbox 架构不兼容（SSR 是 mihomo 内核能力边界，非面板缺陷）。面板上传导入已加**类型分布提示**：SSR 等不被支持类型导入时明确提示"N 个节点均为 SSR（ShadowsocksR），客户端 mihomo 内核不支持，无法上架"，而非笼统 0 节点。**多协议接入（kuaiyu 实测打通）**：上游常按 UA 返回不同格式（浏览器 UA=base64 v2rayN 链接列表 anytls/hysteria2/ss，clash UA=Clash YAML）；服务端新增 `parseNodeConfig` 统一解析（YAML / base64 包裹 YAML / base64 v2rayN 链接列表三态，URL 解析含 anytls/hysteria2/ss/vmess/vless/trojan，localhost 占位伪节点自动过滤）；`rebuildNodes` 支持 anytls/hysteria(v2)/shadowsocks/vmess 多协议入库（SSR 跳过提示）；真实链接密码走 tags `relay-password:` 载体（Xboard `PROTOCOL_CONFIGURATIONS` schema 白名单会过滤 protocol_settings 自定义字段，实测验证）；**hysteria2 salamander 混淆密码同样走 tags `relay-obfs:` 载体**（schema cast 会把空值转 null，2026-09-04 修复；clash YAML 响应字段为连字符 `obfs-password`，解析需兼容 `obfs_password`/`obfs-password` 双键名）；ClashMeta 补丁对 buildAnyTLS/buildHysteria/buildShadowsocks 统一 tags 密码 override + hy2 up/down 空值跳过 + hy2 obfs-password override；所有 relay 增删查改移除 `type=vmess` 限制（按 relay-src 标记跨协议）。**运营手动锁定态（2026-09-04 修复）**：手动「下架」记录源级 `manual_hidden`（upstreams 持久化，normalize 须保留该字段——此前被剥离导致锁定失效），`rebuildNodes` 重建与 `applyExhaustPolicy`（有量自动上架）均尊重锁定不恢复；手动「上架」解除锁定。实测：kuaiyu 260GB 账号 20 节点上架（11 anytls+8 hysteria+1 ss，含 CN2 三网优化/家宽地区节点，客户端 kRegionRules 可直接聚合）。**自定义关键词过滤（2026-09-04）**：全局 `filter_keywords` 配置（`GET/POST /settings`，面板源管理页维护，逗号/换行分隔），`rebuildNodes` 对命中关键词（`mb_stripos` 大小写不敏感）的节点名直接跳过不参与上架，`counts.filtered` 统计并在导入/同步消息提示"已按关键词过滤 N 个节点"；典型用途过滤信息伪节点（剩余流量/套餐时间/官网地址/应急）与测试节点。容器内实测：设「剩余流量,测试」→ kuaiyu 24 节点过滤 9 个、上架 11、库内零残留。**settings 独立字段提交（2026-09-04 修复）**：`POST /settings` 各字段（sync_interval_minutes / filter_keywords / auto_hide_exhausted）独立可选、谁提交谁生效互不覆盖——此前周期被当必填（缺省 0 触发校验失败），导致只保存关键词的请求被拒、关键词未落库、随后操作被前端空值覆盖输入框（表现为"保存无反应/关键词消失"）；已实测仅关键词、仅周期、双字段均 200 且互不影响。
