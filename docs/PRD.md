...
**寄生中转插件 CloudBridgeRelay**（2026-09-01 上线，测试面板 `/opt/xboard-test/plugins/CloudBridgeRelay/`，已启用）：一键导入上游机场订阅为面板寄生节点（vmess+ws，去重、地域脱敏重命名，上游 uuid/cipher 经 `tags` 列载体 + ClashMeta.buildVmess / ServerService vmess 双补丁覆盖下发）。管理：`php artisan cloudbridge:relay sync|status|hide|show`（`sync --url=` 一键填入订阅链接）；定时自动同步（间隔可配，默认 360 分钟节流）；每次同步记录上游 `subscription-userinfo` 流量水位，耗尽自动下架寄生节点（恢复自动上架）。实测：上游账号流量 700GB 已用 669GB（剩余 30.82GB，9-13 到期）——此前节点全超时即因流量耗尽；"55 节点"去重后仅 2 个唯一出口（同服务器多马甲）。**寄生合规提醒**：转售上游节点可能违反上游 ToS，商用需评估。
