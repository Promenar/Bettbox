...
日期：2026-09-22。状态：邀请客户端候选已通过本地验证；测试入口与邀请码预填已验证；后台分享域名配置待更正，账户业务闭环待验收；平台扩展为规划。
...
- 公开配置 `app_url=https://cloud.bingcn.site`；该网站在 curl 下 TLS 连接失败，在浏览器下 `ERR_CONNECTION_CLOSED`。客户端按后台配置会生成指向该地址的邀请链接；后台配置或正式公开网站需先确认。
- 拟更正测试面板 `app_url` 为 `https://cloud.microsoftnexushub.top:8443`，仅变更该网站配置，客户端仍遵循后台权威来源；该服务端持久配置变更等待明确授权。
- 未登录后台、未创建真实邀请码、未提交注册、未生成付款或佣金流水；登录态接口、邀请码归属和返佣到账仍待受控测试账号验收。
